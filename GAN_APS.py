...
def SimpleMetrics(y_pred, y_true):
    ...
    print(pd.DataFrame([[TN, FP], [FN, TP]], columns=['Pred 0', 'Pred 1'], index=['True 0', 'True 1']))
...
def load_preprocess_aps_data():
    train_data = pd.read_csv(
        r'../MSC_implementation/datasets/train_processed_aps.csv', index_col=0)
    test_data = pd.read_csv(
        r'../MSC_implementation/datasets/test_processed_aps.csv', index_col=0)

    #X_train = train_data.loc[:, train_data.columns != 'failure'].copy()
    #Y_train = train_data.loc[:, train_data.columns == 'failure'].copy()

    #X_test = test_data.loc[:, test_data.columns != 'failure'].copy()
    #Y_test = test_data.loc[:, test_data.columns == 'failure'].copy()

    train_data['failure'] = train_data['failure'].replace(['neg','pos'],[0,1])
    test_data['failure'] = test_data['failure'].replace(['neg','pos'],[0,1])
    return train_data,test_data

def aps_cost(y_pred, y_true):
    TP, TN, FP, FN = BaseMetrics(y_pred, y_true)
    total_cost = 10 * FP + 500 * FN
    return  total_cost

def check_performance(g_z,train_data,test_data, data_cols, label_cols=[], seed=0, with_class=False, data_dim=2):
    ...
    xgb= XGBClassifier(max_depth=3, n_estimators=18, n_jobs=-1, scale_pos_weight=40, min_child_weight=44)
    ...
    print('Test performance confusion',confusion_matrix(y_test,y_pred)) # assumes balanced real and generated datasets
    return aps_cost(y_pred, y_test)  # assumes balanced real and generated datasets
...
def CheckAccuracy(x, g_z, data_cols, label_cols=[], seed=0, with_class=False, data_dim=2):
    ...
    #print('check accuracy::::',x.shape,g_z.shape)
    #print(int(len(x) / 2),int(len(g_z) / 2) ,x[int(len(x) / 2)-1:].shape)
    dtrain = np.vstack(
    ...
    xgb_params = {
        # 'tree_method': 'hist', # for faster evaluation
        'max_depth': 6,  # for faster evaluation
        'objective': 'binary:logistic',
        'random_state': 0,
        'eval_metric': 'auc',  # allows for balanced or unbalanced classes
        'silent':1
    ...
    #print("APS cost::",aps_cost(y_pred, y_true))  # assumes balanced real and generated datasets
    #print("Confusion matrix::")
...
def PlotData(x, g_z, data_cols, label_cols=[], seed=0, with_class=False, data_dim=2, save=False, prefix=''):
    ...
    if with_class:
        axarr[0].scatter(real_samples[data_cols[0]], real_samples[data_cols[76]],
                         c=real_samples[label_cols[0]] / 2)  # , cmap='plasma'  )
        axarr[1].scatter(gen_samples[data_cols[0]], gen_samples[data_cols[76]],
        ...
        axarr[0].scatter(real_samples[data_cols[0]], real_samples[data_cols[76]])  # , cmap='plasma'  )
        axarr[1].scatter(gen_samples[data_cols[0]], gen_samples[data_cols[76]])  # , cmap='plasma'  )
    axarr[0].set_title('real')
    axarr[1].set_title('generated')
    axarr[0].set_ylabel(data_cols[76])  # Only add y label to left plot
...
def define_models_CGAN(rand_dim, data_dim, label_dim, base_n_count, type=None):
    ...
    generated_image_tensor = generator_network_w_label(generator_input_tensor, labels_tensor, data_dim, label_dim,
    ...
    generator_model = models.Model(inputs=[generator_input_tensor, labels_tensor], outputs=[generated_image_tensor],
    ...
    combined_model = models.Model(inputs=[generator_input_tensor, labels_tensor], outputs=[combined_output],
...
def train_discriminator_step(model_components, seed=0):
    [cache_prefix, with_class, starting_step,
     train,train_df,test_df, data_cols, data_dim,
...
def training_steps_WGAN(model_components):
    ...
    for i in range(starting_step, starting_step + nb_steps):
        ...
        if not i % 100:  # 2x faster than testing each step...
            K.set_learning_phase(0)  # 0 = test
            test_size = 998  # test using all of the actual fraud data
        ...
        if not i % log_interval:
            ...
            # loss summaries
            print('Losses: G, D Gen, D Real, Xgb: {:.4f}, {:.4f}, {:.4f}, {:.4f}'.format(combined_loss[-1],
                                                                                         disc_loss_generated[-1],
                                                                                         disc_loss_real[-1],
                                                                                         xgb_losses[-1]))
            print('D Real - D Gen: {:.4f}'.format(disc_loss_real[-1] - disc_loss_generated[-1]))
            # print('Generator models loss: {}.'.format(combined_loss[-1]))
            # print('Discriminator models loss gen: {}.'.format(disc_loss_generated[-1]))
            # print('Discriminator models loss real: {}.'.format(disc_loss_real[-1]))
            # print('xgboost accuracy: {}'.format(xgb_losses[-1]) )
            performance = check_performance(g_z,train_df,test_df, data_cols, label_cols, seed=0, with_class=with_class,
                                            data_dim=data_dim)
...
def adversarial_training_WGAN(arguments, train,train_df,test_df, data_cols, label_cols=[], seed=0, starting_step=0):
    [rand_dim, nb_steps, batch_size,
     k_d, k_g, critic_pre_train_steps, log_interval, learning_rate, base_n_count,
...
def training_steps_GAN(model_components):
    ...
    for i in range(starting_step, starting_step + nb_steps):
        ...
        if not i % 10:  # 2x faster than testing each step...
            K.set_learning_phase(0)  # 0 = test
            test_size = 998#492  # test using all of the actual fraud data
            x = get_data_batch(train, test_size, seed=i)
            z = np.random.normal(size=(test_size, rand_dim))
            if with_class:
                labels = x[:, -label_dim:]
            ...
            xgb_loss = CheckAccuracy(x, g_z, data_cols, label_cols, seed=0, with_class=with_class, data_dim=data_dim)
            xgb_losses = np.append(xgb_losses, xgb_loss)
            #np.savetxt('g_z.txt',g_z,delimiter=',')
...
def adversarial_training_GAN(arguments, train,train_df,test_df, data_cols, label_cols=[], seed=0, starting_step=0):
    [rand_dim, nb_steps, batch_size,
     k_d, k_g, critic_pre_train_steps, log_interval, learning_rate, base_n_count,
     data_dir, generator_model_path, discriminator_model_path, loss_pickle_path, show] = arguments
